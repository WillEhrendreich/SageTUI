...
module TestHarness =
  let private processCmd (program: Program<'model, 'msg>) (model: 'model) (cmd: Cmd<'msg>) : 'model * bool =
    let rec loop model cmd =
      ...
      | Batch cmds ->
        cmds |> List.fold (fun (m, q) c ->
          match q with
          | true -> m, true
          | false -> loop m c) (model, false)
      | _ -> model, false
    loop model cmd

  let init (width: int) (height: int) (program: Program<'model, 'msg>) : TestApp<'model, 'msg> =
    let model, cmd = program.Init()
    let model, hasQuit = processCmd program model cmd
    { Model = model; Program = program; Width = width; Height = height; HasQuit = hasQuit }

  let private routeKey (key: Key) (mods: Modifiers) (app: TestApp<'model, 'msg>) : TestApp<'model, 'msg> =
    ...
    let msgs =
      [ for sub in subs do
          ...
          | _ -> () ]
    let model, hasQuit =
      msgs |> List.fold (fun (m, q) msg ->
        match q with
        | true -> m, true
        | false ->
          let newModel, cmd = app.Program.Update msg m
          let newModel, quit = processCmd app.Program newModel cmd
          newModel, quit) (app.Model, app.HasQuit)
    { app with Model = model; HasQuit = hasQuit }

  let pressKey (key: Key) (app: TestApp<'model, 'msg>) : TestApp<'model, 'msg> =
    routeKey key Modifiers.None app

  let pressKeyWith (key: Key) (mods: Modifiers) (app: TestApp<'model, 'msg>) : TestApp<'model, 'msg> =
    routeKey key mods app
  ...
  let sendMsg (msg: 'msg) (app: TestApp<'model, 'msg>) : TestApp<'model, 'msg> =
    let newModel, cmd = app.Program.Update msg app.Model
    let newModel, hasQuit = processCmd app.Program newModel cmd
    { app with Model = newModel; HasQuit = hasQuit }

  let renderBuffer (app: TestApp<'model, 'msg>) : Buffer =
...
// ============================================================
// RENDER HELPER (standalone element rendering, no TEA)
// ============================================================
