...
- **PTY raw-input initialization ordering** — Initialize the .NET console writer
  before setting Unix raw mode, so runtime initialization cannot overwrite
  `VMIN`/`VTIME` after the native input reader starts. Treat both CR and LF as Enter.

## [0.9.9] - 2026-08-04

### Fixed

- **Unix raw input after SSH/ConPTY startup** — Explicitly set Linux `VMIN=1` and
  `VTIME=0` for native `read(2)`, and retry transient `EINTR`/`EAGAIN`/`EWOULDBLOCK`
  errors. This prevents a rendered TUI from silently losing all keyboard input.
